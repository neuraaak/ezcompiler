# User guides

Task-oriented guides for configuring and extending **EzCompiler**.

## 📋 Available guides

| Guide                                                              | Goal                                                          |
| :----------------------------------------------------------------- | :------------------------------------------------------------ |
| [How to configure a compiler](configuration.md)                    | Select a backend and configure compilation settings.          |
| [How to contribute changes locally](development.md)                | Set up a local development workflow and quality checks.       |
| [How to set up secure updates with tufup](secure-updates-tufup.md) | Integrate tufup for signed TUF-compliant update repositories. |
| [How to build a Windows installer](windows-installer.md)           | Package a compiled bundle into a setup.exe with Inno Setup.   |
